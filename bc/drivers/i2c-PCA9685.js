...
//      console.log("set defaul servo setting.");
    ...
    return new Promise((resolve, reject)=>{
      this.i2cPort.open(this.slaveAddress).then(async (i2cSlave)=>{
        ...
        console.log("PCA9685.init 02");
        if ( !noSetZero ){
          for ( var servoPort = 0 ; servoPort < 16 ; servoPort ++ ){
            await this.setServo(servoPort , 0 );
          }
        }
        console.log("PCA9685.init finish");
        resolve();
      },(err)=>{
        reject(err);
      });
...
//    console.log(servoPort,angle)
    console.log("setServo port=%d, angle=%d",servoPort,angle)
    const portStart = 8;
